...
SERVER_LOG="${SERVER_LOG:-wrk/run_get_noexists_code.log}"
PORT="${PORT:-8888}"
CODE_FILE="${CODE_FILE:-code.txt}"
PROVIDER=""
...
echo "running: CODE_FILE=${CODE_FILE} wrk -t${THREADS} -c${CONNS} -d${DURATION} -s get_noexists_code.lua ${BASE_URL}"

CODE_FILE="${CODE_FILE}" wrk \
  -t"${THREADS}" \
  -c"${CONNS}" \
  -d"${DURATION}" \
  -s get_noexists_code.lua \
  "${BASE_URL}"
