...
func (g *MySQLAutoIncrementGenerator) NextCode(ctx context.Context, userID *uint64, normalizedURL, urlHash string) (string, error) {
	if g == nil || g.dao == nil {
		return "", errors.New("mysql auto increment generator dao is not configured")
	}

	return g.dao.CreateWithAutoIncrement(ctx, userID, normalizedURL, urlHash)
}
